...
For each development you include: consolidate duplicate reporting into one item, write an ORIGINAL 40-120 word summary in your own words (never copy source text verbatim), include casualty/displacement/outbreak figures only when explicitly present in the material, distinguish verified facts from claims/estimates/forecasts, use neutral factual language, and note the strongest available source.

Score each development 0-10 for HUMANITARIAN significance, not political salience: Watch 1-3, Serious 4-6, Severe 7-8, Critical 9-10.
