...
| Corroboration ("when multiple sources report the same event...") | Two mechanisms, both restricted to `SOURCE_HIERARCHY` outlets only: (1) `scripts/analyze.py` → `dedup_items()` — when merging near-duplicate items found during normal fetching, the primary/representative source is always kept regardless of hierarchy status (preserves standalone minor-outlet coverage), but additional cluster members are only added to `sources` if hierarchy-listed. (2) `scripts/build_edition.py` → `corroborate_report_sources()` — after a report is finalized, re-scans the region's full already-fetched candidate pool (not just the smaller subset offered to the LLM) for additional priority-hierarchy coverage, using the same similarity check as dedup. No new network calls either way. Real coverage rate depends on the similarity threshold (`config.CORROBORATION_SIMILARITY_THRESHOLD`) matching how differently outlets word the same story in practice; this hasn't been tuned against real output yet. |
| Required outlet review (search each named outlet directly) | Two mechanisms: (1) `scripts/config.py` → `OUTLET_RSS_FEEDS` + `scripts/fetch.py` → `fetch_outlet_rss()` for outlets with a real public RSS feed (BBC, Al Jazeera English, The Guardian, France 24 English). (2) `scripts/fetch.py` → `GOOGLE_NEWS_DOMAIN_SOURCES` + `fetch_google_news_domain()` for outlets with no public RSS — Associated Press, Reuters, AFP — using Google News' `allinurl:` search operator scoped to that outlet's domain as a workaround. AP's yield is verified good; Reuters/AFP added on the same pattern but not yet verified live. This was added after a real gap surfaced in practice: a Google-News-only run missed a major typhoon story that BBC's own feed caught immediately. |
